...
size_t insertHelper(const std::vector<T>& elements, const C& val, size_t a, size_t b){
  ...
  while (a+1 != b|| a != b )
  {
    ...
    if (val > elements[track] && val < elements[track+1]) return track+1;
    if (val > elements[track-1] && val < elements[track]) return track;
    if (elements[track] == val) return track;
    if (val > elements[track]){
      a=track;
      track = (a+b)/2;
    }
    else if(val < elements[track]){
      b=track;
      track = (a+b)/2;
    }
  }
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    return insertHelper(elements, val, 0, elements.size()-1);
}
