...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
  ...
  unsigned cur=hashes::hash(key, size);
  unsigned diff=hashes::secondary_hash(key,size);
  while (table[cur] != NULL)
  {
    cur=(cur+diff)%size;
  }
  table[cur]=new std::pair<K,V>(key, value);
  should_probe[cur]=true;
...
void DHHashTable<K, V>::remove(K const& key)
{
    ...
    //unsigned jump=hashes::secondary_hash(key,size);
    bool boo=true;
    curr=findIndex(key);
    if (curr==-1)return;
    if (table[curr]==NULL)boo=false;
    if (boo)
    {
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
  ...
  {
    if (table[curr]!= NULL && table[curr]->first==key)return curr;
    curr=(curr+jump)%size;
    if (start==curr)break;
  }
    return -1;
