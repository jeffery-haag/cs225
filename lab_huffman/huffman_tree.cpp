...
{
  HuffmanTree::TreeNode*sec=singleQueue.front();
  HuffmanTree::TreeNode*first=mergeQueue.front();

  if (first->freq.getFrequency()<sec->freq.getFrequency()  )
  {
    mergeQueue.pop();
    //delete sec;
    return first;
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    {
      if (frequencies[i].getCharacter() != '\0')
      {
        TreeNode* newb= new TreeNode(frequencies[i]);
        i++;
        singleQueue.push(newb);
      }
      else
      {
        TreeNode* newb= new TreeNode(frequencies[i]);
        i++;
        mergeQueue.push(newb);
      }
    }
    ...
    while (singleQueue.size()+mergeQueue.size()>1)
    {
      ...
      sum=temp->freq.getFrequency();

      TreeNode* temp2=removeSmallest(singleQueue, mergeQueue);
      sum=temp->freq.getFrequency()+sum;

      TreeNode* parent=new TreeNode(sum);
      parent->right=temp2;
      parent->left=temp;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
  return NULL;
    ...
    if (bfile.getNextBit())
    {
      TreeNode* temp=new TreeNode(bfile.getNextByte());
      return temp;
    }
    else
    {
